...
const CustomLink = ({ href, title, className = "" }) => {
  ...
  return (
    <Link href={href} className={`${className} relative group`}>
      {title}
      <span
        className={`h-[1px] line-block bg-dark absolute left-0 -bottom-0.5 group-hover:w-full transition-[width] ease duration-300 ${
...
const CustomMobileLink = ({ href, title, className = "", toggle }) => {
  ...
  return (
    ...
    >
      {title}
      <span
        className={` bg-light h-[1px] line-block dark:bg-dark absolute left-0 -bottom-0.5 group-hover:w-full transition-[width] ease duration-300 ${
          router.asPath === href ? "w-full" : "w-0"
        } `}
      >
        &nbsp;
    ...
    <header className="w-full px-32 py-8 font-medium flex items-center justify-between dark:text-light relative z-10 lg:px-16 md:px-12 sm:px-8">
      ...
      <div className="w-full flex  justify-between items-center lg:hidden">
        ...
        <nav className="flex items-center justify-center flex-wrap">
          <motion.a
            href="https://github.com/hossameldinfahmi"
            target={"_blank"}
            whileHover={{ y: -2 }}
            whileTap={{ scale: 0.9 }}
            className="w-6 mx-3  rounded-full dark-bg-dark"
          >
            <GithubIcon />
          </motion.a>
          <motion.a
            href="https://www.linkedin.com/in/hossamfahmi/"
            target={"_blank"}
            whileHover={{ y: -2 }}
            whileTap={{ scale: 0.9 }}
            className="w-6 mx-3"
          ...
          <motion.a
            href="https://leetcode.com/hossamfahmy/"
            target={"_blank"}
            whileHover={{ y: -2 }}
            whileTap={{ scale: 0.9 }}
            className="w-6 mx-3"
      ...
      {isOpen ? (
        <motion.div
          ...
          className="z-30 bg-dark/90 dark:bg-light/75 rounded-lg backdrop-blur-md py-32 min-w-[70vw] flex flex-col justify-between items-center fixed left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2"
        >
          {" "}
          <nav
          ...
          <nav className="flex items-center justify-center flex-wrap mt-2 sm:mx-1">
            ...
            <motion.a
              href="https://twitter.com/fahmyuiux"
              target={"_blank"}
              whileHover={{ y: -2 }}
              whileTap={{ scale: 0.9 }}
